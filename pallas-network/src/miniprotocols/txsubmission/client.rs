    ...
    Message<TxId, TxBody>: Fragment,
{
    ...
    pub async fn send_message(&mut self, msg: &Message<TxId, TxBody>, assert_state: bool) -> Result<(), Error> {
        if assert_state {
    ...
    pub async fn recv_message(&mut self, assert_state: bool) -> Result<Message<TxId, TxBody>, Error> {
        if assert_state {
